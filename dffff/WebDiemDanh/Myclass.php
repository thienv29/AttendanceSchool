...
<body>
    ...
    <div class="modal ">
        ...
        <div class="modal-div">
            ...
            if ($count == 0) {
                ...
                $sql = "SELECT * FROM tbl_class WHERE id_teacher='$idteacher'";
                ...
                if (mysqli_num_rows($old) > 0) {
                    while ($row  = $old->fetch_assoc()) {
                        ...
                        echo '<div class="modal-main-chat ' . $temp . '" idclass=' . $row['id_class'] . '>
                            ...
                            <div class="modal-main-chat-box-body">                        
                        </div>
            ...
            <div class="modal-main">
                <img src="" alt="" id="imgQr" srcset="">
                <!-- <span id="countdown">Giới hạn thời gian 60 giây</span> -->
                <div class="course-percent">
                    <svg height="100" width="100">
                        ...
                        <circle class="course-percent-circle" cx="50" cy="50" r="35" stroke="lime" stroke-width="8" fill="transparent" style="stroke-dasharray: 220; stroke-dashoffset: 0;"></circle>

                    </svg>
                    <span class="course-percent-text" id="countdown">30s</span>
                </div>
            </div>
            ...
            <div class="attendanceNote grid-col-3" data-aos="fade-right">
                <div class="attendanceNote-profile">
                    <div class="attendanceNote-profile-img">
                        <img src="images/iconteacher.png" alt="" srcset=" ">
                    </div>
                ...
                <div class="attendanceNote-class active">
                    ...
                    include('config.php');
                    $idteacher = $_SESSION['idteacher'];
                    $sql = "SELECT tbl_class.subject,tbl_class.id_class,tbl_class.dayOfWeek,tbl_class.timeStart,tbl_class.timeOver FROM tbl_teacher,tbl_class where tbl_class.id_teacher=tbl_teacher.id_giaovien and tbl_teacher.id_giaovien='$idteacher'";
                    $old = mysqli_query($conn, $sql);
                    $countClass = 0;
                    $listIdClass = array();
                    if (mysqli_num_rows($old) > 0) {
                ...
                <div class="hasClass grid-col-12">
                    <div class="actionClass gridrow">
                        ...
                        <div class="btn-action grid-col-3">
                            <button class="btn-action-attendance btn-getstarted" datatbl="0" onclick="">
                                Attendance
                                <span class="attendanceErr"></span>
                            </button>
    ...
    <script type="text/javascript">
        ...
        //     xmlhttp.open("GET", "randomString.php?idclass=" + listIdClass[index] + "&count=" + index, true);

        //     xmlhttp.send();

        // }
        var intervalId
        axc = document.querySelector(".btn-action-attendance")
        ...
        btnChat.onclick = function() {
            ...
            intervalId = window.setInterval(function() {
                ...
                var mess = document.querySelectorAll(".modal-main-chat.active .modal-main-chat-box-body .message");
        ...
        axc.onclick = function() {

            var index = document.querySelector(".attendanceNote-class-item.active").getAttribute("idclass");

            var today = new Date();

            var days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
            stringtime = document.querySelector(".attendanceNote-class-item.active .attendanceNote-class-item-text .attendanceNote-class-item-time ").innerHTML
            listStringTime = stringtime.slice(1, stringtime.length - 1).split(" ")
            var hourStart = parseInt(listStringTime[1].split(":")[0]);
            var minusStart = parseInt(listStringTime[1].split(":")[1]);
            var hourOver = parseInt(listStringTime[2].split(":")[0]);
            var minusOver = parseInt(listStringTime[2].split(":")[1]);
            if (days[today.getDay()] != listStringTime[0] || hourStart > today.getHours() || hourOver < today.getHours) {
                document.querySelector(".attendanceErr").innerHTML = "Time out!!!"
            } else {
                var xmlhttps = new XMLHttpRequest();

                xmlhttps.open("GET", "teacherAttendace.php?idclass=" + index, true);

                xmlhttps.send();

                modal.classList.add("modalAttendance");
                if (countBtn == 0) {
                    var timeleft = 30;
                    var downloadTimer = setInterval(function() {
                        if (timeleft < 0) {
                            ...
                            xmlhttp.open("GET", "randomString.php?idclass=" + index, true);

                            xmlhttp.send();
                            modal.style.display = "none"
                            modal.classList.remove("modalAttendance")
                        }
                        ...
                        percentCircle.style.strokeDashoffset = `calc(220 - (220*${percentInt})/30`
                        if (timeleft < 10) {
                            percentCircle.style.stroke = "red";
                        } else {
                            percentCircle.style.stroke = "lime";
                        }
                        document.getElementById("countdown").innerHTML = timeleft + "s";
                        timeleft -= 1;
                    }, 1000);

                    document.querySelector('#imgQr').setAttribute("src", `images/qr-ima${index}.png`)
                    modal.style.display = "flex"
                    countBtn = 1;
                }
            }
        ...
        for (let index = 0; index < listModalChat.length; index++) {
            ...
            element.querySelector(".btn-send").onclick = function() {
                ...
                if (string != "") {
                    ...
                    const distinv2 = "<?php echo $_SESSION['user'] ?>"

                    time = `${today.getFullYear()}-${today.getMonth()}-${today.getDay()} ${today.getHours()}:${today.getMinutes()}:${today.getSeconds()}`
                    var xmlhttp = new XMLHttpRequest();

                    xmlhttp.open("GET", "sendMessage.php?idclass=" + idclass + "&time=" + time + "&message=" + string + "&name=" + name + "&distin=" + 1 + "&distinv2=" + distinv2, true);
